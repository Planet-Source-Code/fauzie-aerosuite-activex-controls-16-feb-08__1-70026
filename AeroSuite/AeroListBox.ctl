VERSION 5.00
Begin VB.UserControl AeroListBox 
   ClientHeight    =   1425
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   2010
   ...
   ScaleMode       =   3  'Pixel
   ScaleWidth      =   134
   ToolboxBitmap   =   "AeroListBox.ctx":0000
   Begin AeroSuite.AeroScrollbar Bar 
      Height          =   1455
      Left            =   1680
      TabIndex        =   0
      Top             =   0
...
Private Sub DrawItem(ByVal Index As Integer)
    ...
    '-- Draw text...
    DrawText UserControl.hdc, m_List(Index).Text, Len(m_List(Index).Text), mTmpRct, DT_SINGLELINE Or DT_VCENTER
...
'-- Item out of area ?
    ...
    RoundRect UserControl.hdc, m_ItemRct(Index - Bar).X1, m_ItemRct(Index - Bar).Y1, m_ItemRct(Index - Bar).X2, m_ItemRct(Index - Bar).Y2, 2, 2
