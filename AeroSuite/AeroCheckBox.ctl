...
Private m_Value As eCheckBoxValue

Private CheckPics As New pcMemDC
...
Private Sub UserControl_Initialize()
  CheckPics.CreateFromPicture LoadResPicture("CHECKBOX", vbResBitmap)
  isShown = True
End Sub
...
Public Property Get Value() As eCheckBoxValue
...
Public Property Let Value(ByVal NewValue As eCheckBoxValue)
  m_Value = NewValue
  Call Redraw(0, True)
...
Private Sub Redraw(ByVal curStat As Byte, ByVal Force As Boolean)
  ...
  i = IIf(m_Value, IIf(m_Value = 1, 52, 104), 0)
  UserControl.Cls
  If m_Enabled Then
    If curStat = 0 And isOver Then curStat = 1
    CheckPics.Draw UserControl.hdc, 0, (curStat * 13) + i, 13, 13, CheckRect.Left, CheckRect.Top
  Else
    CheckPics.Draw UserControl.hdc, 0, (3 * 13) + i, 13, 13, CheckRect.Left, CheckRect.Top
  End If
  Call DrawCaption
  DrawFocusR
End Sub
